...
class Chao(commands.Cog):
    def __init__(self, bot):
        ...
        # Initialize utilities as None; they will be set in cog_load
        self.image_utils = None
        self.data_utils = None

    def cog_load(self):
        # Access the utility cogs
    ...
    async def initialize_inventory(self, ctx, guild_id, user_id, embed_title, embed_desc):
        ...
        self.data_utils.save_inventory(
            ...
            self.data_utils.load_inventory(self.data_utils.get_path(guild_id, user_id, 'user_data', 'inventory.parquet')),
    ...
    async def give_rings(self, ctx):
        ...
        self.data_utils.save_inventory(file_path, inventory_df, current_inventory)
    ...
    async def hatch(self, ctx):
        ...
        chao_path = os.path.join(self.data_utils.get_path(guild_id, user_id, 'chao_data', chao_name), f'{chao_name}_stats.parquet')
        os.makedirs(os.path.dirname(chao_path), exist_ok=True)
        inventory_df.at[inventory_df.index[-1], 'Chao Egg'] = chao_egg_quantity - 1
        self.data_utils.save_inventory(file_path, inventory_df, inventory_df.iloc[-1].to_dict())
        ...
        chao_stats = {
            ...
            **{f"{stat}_grade": 'D' for stat in ['power', 'swim', 'stamina', 'fly', 'run', 'mind']},
            'evolved': 0,  # Changed from False to 0
            'Type': 'Normal',
            'swim_fly': 0,
            'run_power': 0,
            'dark_hero': 0,
        ...
        chao_df = self.data_utils.load_chao_stats(chao_path)
        self.data_utils.save_chao_stats(chao_path, chao_df, chao_stats)
        ...
        # Combine images with face using image_utils
    ...
    async def buy(self, ctx, *, item_quantity: str):
        ...
        file_path = self.data_utils.get_path(guild_id, user_id, 'user_data', 'inventory.parquet')
        inventory_df = self.data_utils.load_inventory(file_path)
        rings = inventory_df.iloc[-1]['rings']
        fruit = next((fruit_item for fruit_item in self.fruits if fruit_item['name'].lower() == item_name.lower()), None)
        if not fruit:
    ...
    async def restore(self, ctx, *, args: str):
        ...
        if parts[0].lower() == 'inventory':
            ...
            # Update or append the restored entry
    ...
    async def stats(self, ctx, *, chao_name: str):
        ...
        # Use image_utils to generate the stats image
        chao_dir = self.data_utils.get_path(guild_id, user_id, 'chao_data', chao_name)
    ...
    async def feed(self, ctx, *, chao_name_and_fruit: str):
        ...
        for i in range(1, len(parts) + 1):
            ...
            fruit_name_match = next(
                (fruit_item['name'] for fruit_item in self.fruits if fruit_item['name'].lower() == potential_fruit_name.lower()),
                None
            )
            if fruit_name_match:
        ...
        chao_dir_path = self.data_utils.get_path(guild_id, user_id, 'chao_data', chao_name)
        ...
        fruit = next(
            (fruit_item for fruit_item in self.fruits if fruit_item['name'].lower() == fruit_name.lower()),
            None
        )
        ...
        print(f"[feed] Before feeding {fruit_name.capitalize()} | Chao: {chao_name} | "
              f"{FRUIT_STATS[fruit_name]}_ticks: {latest_stats.get(f'{FRUIT_STATS[fruit_name]}_ticks', 0)} | "
              f"Level: {latest_stats.get(f'{FRUIT_STATS[fruit_name]}_level', 0)} | "
              f"swim_fly: {latest_stats.get('swim_fly', 0)} | run_power: {latest_stats.get('run_power', 0)}")
        ...
        level_key = f"{FRUIT_STATS[fruit['name']]}_level"
        ...
        # Log after feeding
        print(f"[feed] After feeding {fruit_name.capitalize()} | Chao: {chao_name} | "
              f"{stat_key}: {latest_stats[stat_key]} | Level: {latest_stats.get(level_key, 0)} | "
              f"swim_fly: {latest_stats.get('swim_fly', 0)} | run_power: {latest_stats.get('run_power', 0)} | "
              f"dark_hero: {latest_stats.get('dark_hero', 0)}")
