...
class ChaoCommands(commands.Cog):
    ...
    async def inventory(self, ctx):
        ...
        embed = discord.Embed(title=f"{ctx.author.name}'s Inventory", description="Here's what you have:", color=self.embed_color)

        inventory_items = []

        for fruit in self.black_market_cog.fruits:
    ...
    async def market(self, ctx):
        ...
        fruits = [dict(fruit, price=15) for fruit in self.black_market_cog.fruits]
